...
'''
model.ingredientCost = Param(model.i, within=PositiveReals)
model.manhourNumber = Param(model.j, within=PositiveReals)
model.oneManhourCost = Param(model.j, within=PositiveReals)
...
model.penaltyPlus(model.i, initialize=surplusInit)

def defInit(model, i):
	return 1.2*(model.dishCost[i] + model.manhourCost[i])

model.penaltyMinus(model.i, initialize=defInit)
...
'''
State model's variables (w and v are for unmet demand/surplus)
'''
...
def generationRule2(model, omega):
	return model.vP[omega] - model.vN[omega] = sum(j * model.y[j] for j in model.j) - model.generalDemand[omega]
...
def objectiveExpr(model):
	return summation(model.manhourCost, model.x) + summation(model.dishCost, model.x) \\
		...
		+ summation(model.tableCost, model.j) \\
